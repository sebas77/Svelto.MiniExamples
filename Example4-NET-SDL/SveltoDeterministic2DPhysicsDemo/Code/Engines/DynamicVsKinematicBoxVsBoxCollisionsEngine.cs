...
namespace MiniExamples.DeterministicPhysicDemo.Physics.Engines
{
    public class DynamicVsKinematicBoxVsBoxCollisionsEngine: IQueryingEntitiesEngine, IScheduledPhysicsEngine
    {
        ...
        {
            var kinematicEntities =
                    entitiesDB
                           .QueryEntities<TransformEntityComponent, RigidbodyEntityComponent,
                                BoxColliderEntityComponent>(GameGroups.KinematicRigidBodyWithBoxColliders.Groups);
            var dynamicEntities =
                    entitiesDB
                           .QueryEntities<TransformEntityComponent, RigidbodyEntityComponent,
                                BoxColliderEntityComponent>(GameGroups.DynamicRigidBodyWithBoxColliders.Groups);
                     ...
                     kinematicEntities)
            {
                for (var kinematicIndex = 0; kinematicIndex < kinematicCount; kinematicIndex++)
                {
                    ...
                    foreach (var ((dynamicTransforms, dynamicRigidBodies, dynamicColliders, dynamicCount), _) in
                             dynamicEntities)
                    {
