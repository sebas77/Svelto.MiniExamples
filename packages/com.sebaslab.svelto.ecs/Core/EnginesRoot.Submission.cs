...
namespace Svelto.ECS
{
    public partial class EnginesRoot
    {
        ...
        {
            var sampler = new PlatformProfiler("remove whole group");
        ...
        {
            var sampler = new PlatformProfiler("swap whole group");
            using (sampler.Sample())
            {
        ...
        {
            var sampler = new PlatformProfiler("remove Entities");
            using (sampler.Sample())
            {
        ...
        {
            var sampler = new PlatformProfiler("Swap entities between groups");
            using (sampler.Sample())
            {
                        ...
                        using (sampler.Sample("Add entities to database"))
                        {
                            ...
                            foreach (var groupToSubmit in _groupedEntityToAdd)
                            {
                                ...
                                foreach (var entityComponentsToSubmit in groupToSubmit.components)
                                {
                                    ...
                                    Span<uint> toIndices = stackalloc uint[fromDictionary.count];
                                    fromDictionary.AddEntitiesToDictionary(
                                        toDictionary, groupID, ref toIndices
        ...
        void SwapEntitiesBetweenGroups(ExclusiveGroupStruct fromGroupId, ExclusiveGroupStruct toGroupId, PlatformProfiler platformProfiler)
        {
            ...
            foreach (var dictionaryOfEntities in fromGroup)
            {
                ...
                fromDictionary.AddEntitiesToDictionary(
                    toDictionary, toGroupId, ref toIndices
#if SLOW_SVELTO_SUBMISSION
                  , entityLocator
#endif
                );

                if (fromDictionary.entityIDs is NativeEntityIDs nativeEntityIds)
                {
                    SwapEntitiesBetweenPersistentFilters(fromGroupId, toGroupId, toIndices, refWrapperType, nativeEntityIds);
                }
                else if (fromDictionary.entityIDs is ManagedEntityIDs managedEntityIds)
                {
                    SwapEntitiesBetweenPersistentFilters(fromGroupId, toGroupId, toIndices, refWrapperType, managedEntityIds);
                }
                else
                {
                    // Should never happen
