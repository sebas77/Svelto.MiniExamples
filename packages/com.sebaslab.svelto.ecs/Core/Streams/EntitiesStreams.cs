...
namespace Svelto.ECS
{
    ...
    ///     through a thread-safe, unbounded concurrent queue.
    ...
    {
        internal Consumer<T> GenerateConsumer<T>(string name)
            where T : unmanaged, _IInternalEntityComponent
        {
            ...
            return (_streams[componentId] as EntityStream<T>).GenerateConsumer(name);
        }
            ...
            where T : unmanaged, _IInternalEntityComponent
        {
            ...
            return typeSafeStream.GenerateConsumer(group, name);
        }
