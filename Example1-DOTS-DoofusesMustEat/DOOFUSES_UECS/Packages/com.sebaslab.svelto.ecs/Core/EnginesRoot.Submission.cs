                    ...
                    for (var i = 0; i < count; i++)
                    {
                        try
                        {
                            switch (entitiesOperations[i].type)
                            {
                                case EntitySubmitOperationType.Swap:
                                    SwapOrRemoveEntities(entitiesOperations[i].builders
                        ...
                        catch
                        {
                            var str = "Crash while executing Entity Operation ".FastConcat(
                                entitiesOperations[i].type.ToString()).FastConcat(" from : ", entitiesOperations[i].trace);

                            Svelto.Console.LogError(str);
