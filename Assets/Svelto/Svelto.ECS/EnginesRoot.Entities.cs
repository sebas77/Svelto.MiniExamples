      ...
      //              groupBufferB[entityViewType] = entityViewBuilder.Preallocate(ref dbList, size);
        ...
        void MoveEntity(IEntityBuilder[] entityBuilders, EGID entityGID, Type originalDescriptorType, EGID? toEntityGID)
        {
            ...
            using (profiler.StartNewSession("Move Entity"))
            {
                ...
                if (_groupEntityDB.TryGetValue(entityGID.groupID, out var fromGroup) == false)
                    throw new ECSException("from group not found eid: ".FastConcat(entityGID.entityID)
                ...
                if (fromGroup.TryGetValue(ENTITY_INFO_VIEW_TYPE, out var entityInfoViewDic) &&
                    (entityInfoViewDic as TypeSafeDictionary<EntityInfoView>).TryGetValue(
                        entityGID.entityID, out entityInfoView) && (correctEntityDescriptorFound =
                        entityInfoView.type == originalDescriptorType))
                {
                    ...
                    for (int i = 0; i < entitiesToMove.Length; i++)
                        MoveEntityView(entityGID, toEntityGID, toGroup, ref fromGroup,
            ...
            ref Dictionary<Type, ITypeSafeDictionary> fromGroup, Type entityViewType, PlatformProfiler profiler)
        {
            ...
            if (toEntityGID != null)
            {
                ...
                groupedGroup[toEntityGID.Value.groupID] = dictionaryOfEntities;
            }
            ...
            fromTypeSafeDictionary.MoveEntityFromDictionaryAndEngines(entityGID, toEntityGID.Value,
                                                                      dictionaryOfEntities, _entityEngines,
                                                                      ref profiler);
